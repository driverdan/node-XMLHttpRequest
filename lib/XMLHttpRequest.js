...
exports.XMLHttpRequest = function() {
  ...
  /**
   * Support overriding the MIME type in order to specify a custom character set
   * so that we treat the data from the server as binary instead of utf8.  This
   * is to conform with the idiom documented at:
   *
  ...
  this.overrideMimeType = function(mimetype) {
    var match = /^([^;]+); charset=(.+)$/.exec(mimetype);
    if (match && match[2] === "x-user-defined")
      charset = "binary";
  };
